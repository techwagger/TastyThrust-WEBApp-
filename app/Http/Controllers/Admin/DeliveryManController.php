...
class DeliveryManController extends Controller
{
    ...
    public function reviews_list(Request $request): Renderable
    {
       ...
       // $reviews = $reviews->with(['delivery_man', 'customer'])->latest()->paginate(Helpers::getPagination())->appends($query_param);
        $reviews = $reviews->with(['delivery_man', 'customer'])->latest()->get();
    ...
    public function store(Request $request): RedirectResponse
    {
        $request->validate([
            ...
            'phone' => 'required|min:7|max:15|unique:delivery_men,phone',
             'identity_number' => 'required|unique:delivery_men,identity_number',
            'confirm_password' => 'same:password'
        ], [
            'f_name.required' => translate('First name is required!'),
            'identity_number.required' => translate('Identity Number is required!')
        ]);
    ...
    public function update(Request $request, $id): RedirectResponse
    {
        ...
        if ($delivery_man['identity_number'] != $request['identity_number']) {
            $request->validate([
                'identity_number' => [
                'required',
                Rule::unique('delivery_men', 'identity_number')->ignore($id)
            ],
    ...
    public function pending_list(Request $request): Factory|View|Application
    {
       ...
       // $delivery_men = $delivery_men->where('application_status', 'pending')->latest()->paginate(Helpers::getPagination())->appends($query_param);
        $delivery_men = $delivery_men->where('application_status', 'pending')->latest()->get();
    ...
    public function denied_list(Request $request): Factory|View|Application
    {
        ...
        $delivery_men = $delivery_men->where('application_status', 'denied')->latest()->get();
